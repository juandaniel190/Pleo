...
function getPeriodRange(period, offset) {
  if (period === 'alltime') {
    // Spans the data range — 2024-01-01 → AS_OF.
    return { start: new Date(2024, 0, 1), end: AS_OF, label: 'All time' };
  }
  if (period === 'quarter') {
...
function getBuckets(period, range) {
  if (period === 'alltime') {
    // One bucket per quarter from the range start through AS_OF.
    const buckets = [];
    const startYear = range.start.getFullYear();
    const endYear   = AS_OF.getFullYear();
    const endQ      = Math.floor(AS_OF.getMonth() / 3);
    for (let y = startYear; y <= endYear; y++) {
      const qMax = (y === endYear) ? endQ : 3;
...
function ArrTrendChart({ data, compact,
  ...
  // All-time spans the data range — the comparison-style line view (current
  // vs prior vs same-Q-LY) doesn't apply, so we force bar mode and disable
  // prev/next navigation in that case.
  const isAllTime = period === 'alltime';
  const effChart  = isAllTime ? 'bar' : chart;
  ...
  const controls = (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center',
      ...
      <div style={{ display: 'inline-flex', gap: 2, background: T.panel, padding: 2, borderRadius: 999,
                    opacity: isAllTime ? 0.45 : 1 }}>
        <PillBtn active={effChart === 'line'} onClick={() => !isAllTime && setChart('line')}>Lines</PillBtn>
        <PillBtn active={effChart === 'bar'}  onClick={() => !isAllTime && setChart('bar')}>Bars</PillBtn>
      </div>
      <div style={{ display: 'inline-flex', gap: 2, background: T.panel, padding: 2, borderRadius: 999 }}>
        ...
        <PillBtn active={period === 'alltime'} onClick={() => setPeriodSafe('alltime')}>All time</PillBtn>
      </div>
      <div style={{ marginLeft: 'auto', display: 'inline-flex', gap: 4, alignItems: 'center' }}>
        <Arrow disabled={!canPrev || isAllTime} onClick={() => !isAllTime && canPrev && setOffset(offset - 1)}>‹</Arrow>
        <span style={{ ...numStyle, fontSize: 12, fontWeight: T.fontWeightMedium, color: T.black, minWidth: 64, textAlign: 'center' }}>
          {range.label}
        </span>
        <Arrow disabled={!canNext || isAllTime} onClick={() => !isAllTime && canNext && setOffset(offset + 1)}>›</Arrow>
      </div>
    </div>
  );
  ...
  // Bar data enriched with attainment % per bucket — so the "answer" sits
  // directly above each bar pair instead of forcing the eye to do the math.
  const barDataPct = barData.map(b => ({
    ...b,
    attainPct: b.target > 0 ? Math.round((b.actuals / b.target) * 100) : null,
  }));
  ...
  return (
    <Card style={{ padding: cardPad }}>
      ...
      {effChart === 'line' ? (() => {
        ...
        <ResponsiveContainer width="100%" height={chartHeight}>
          <BarChart data={barDataPct} margin={{ top: 36, right: 10, left: -10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="2 4" vertical={false} stroke={T.chartGrid} />
            <XAxis dataKey="label" tick={{ fontSize: isAllTime ? 9 : 11, fill: T.textMuted }}
                   axisLine={false} tickLine={false}
                   interval={0} />
            <YAxis tickFormatter={(v) => `€${Math.round(v/1000)}K`}
                   tick={{ fontSize: 10, fill: T.textMuted }} axisLine={false} tickLine={false} />
            <Tooltip formatter={(v, key) =>
                      key === 'attainPct' ? `${v}%` : `€${Math.round(v/1000).toLocaleString()}K`}
                     contentStyle={TooltipBox} labelStyle={TooltipLabel} itemStyle={TooltipItem} />
            <Legend wrapperStyle={{ fontSize: 11 }} formatter={legendBlackFmt}
                    payload={[
                      { value: 'Target',  type: 'rect', color: T.hairline },
                      { value: 'Actuals', type: 'rect', color: T.pleoPink500 },
                    ]} />
            {/* Target bars (light grey, behind) — value label only when not All-time
                so the all-time view doesn't get a wall of €K labels. */}
            ...
            {/* Actuals bars (pink) — attainment % labelled above; €K still labelled
                on quarterly/monthly views where there's room. */}
            <Bar dataKey="actuals" name="Actuals" fill={T.pleoPink500} radius={[3,3,0,0]}>
              <LabelList dataKey="attainPct" position="top" offset={isAllTime ? 4 : 18}
                         fontSize={isAllTime ? 10 : 11} fontWeight={700} fill={T.black}
...
function StaleBySegmentChart({ data, compact }) {
  // Heatmap layout (rows = market, cols = segment) — same pattern as the
  // executive deck's slide 2. Replaces the horizontal bar chart because
  // the grid surfaces the structural concentration ("Mid-Market is the
  // engine") at a glance, where the bar chart only showed top 6 cells.
  //
  // Cell colour is bucketed by ARR magnitude using the Pleo pink ramp
  // (100 → 400). Empty cells render as a dashed-border placeholder so
  // sparse markets stay legible.
  const cells = data.staleBySegment || [];
  if (!cells.length) {
    return (
      <Card style={{ padding: 14, fontSize: 12, color: T.textMuted }}>
        No stale ARR in the data.
      </Card>
    );
  }
  const SEGMENTS = ['SMB', 'Mid-Market', 'Enterprise'];
  // Markets ordered by total stale ARR (biggest concentration first).
  const marketTotals = new Map();
  for (const c of cells) {
    marketTotals.set(c.market, (marketTotals.get(c.market) || 0) + (c.arr_keur || 0));
  }
  const MARKETS = [...marketTotals.entries()].sort((a, b) => b[1] - a[1]).map(([m]) => m);
  // Per-segment totals (for the totals row below).
  const segTotals = Object.fromEntries(SEGMENTS.map(s => [s, 0]));
  const grandTotal = cells.reduce((a, c) => a + (c.arr_keur || 0), 0) || 1;
  for (const c of cells) if (SEGMENTS.includes(c.segment)) segTotals[c.segment] += c.arr_keur || 0;
  // Index by (market|segment).
  const lookup = new Map();
  for (const c of cells) lookup.set(`${c.market}__${c.segment}`, c);
  // Bucketize cell intensity (pink-100 → pink-400) by ARR magnitude.
  const max = Math.max(1, ...cells.map(c => c.arr_keur || 0));
  const lvl = (v) => {
    if (!v) return 0;
    const r = v / max;
    if (r >= 0.7) return 4;
  ...
  const headerCell = (label) => (
    <div style={{
      fontSize: 10, color: T.textSec, fontWeight: T.fontWeightSemibold,
      textTransform: 'uppercase', letterSpacing: '0.06em',
      background: T.panel, borderRadius: 6, padding: '6px 8px', textAlign: 'center',
    }}>{label}</div>
  );
  ...
  return (
    <Card style={{ padding: compact ? 12 : 16 }}>
      <div style={{
        display: 'grid', gap: 4,
        gridTemplateColumns: `64px repeat(${SEGMENTS.length}, 1fr)`,
      }}>
        {/* Header row */}
        <div style={{
          fontSize: 9, color: T.textMuted, textTransform: 'uppercase',
          letterSpacing: '0.08em', alignSelf: 'center', padding: '4px 6px',
        }}>Market ↓</div>
        {SEGMENTS.map(s => <React.Fragment key={s}>{headerCell(s)}</React.Fragment>)}

        {/* Data rows */}
        {MARKETS.map(market => (
          <React.Fragment key={market}>
            <div style={{
              fontSize: 11, color: T.textSec, fontWeight: T.fontWeightSemibold,
              display: 'flex', alignItems: 'center', padding: '0 6px',
            }}>{market}</div>
            {SEGMENTS.map(segment => {
              const c = lookup.get(`${market}__${segment}`);
              const v = c ? c.arr_keur : 0;
              const L = lvl(v);
              if (!c) {
                return (
              ...
              return (
                <div key={segment} style={{
                  background: lvlBg[L], border: `1px solid ${lvlBorder[L]}`,
                  borderRadius: 6, padding: '6px 8px',
                  display: 'flex', flexDirection: 'column', gap: 1,
                  ...numStyle,
                }}>
                  <div style={{ fontSize: 13, fontWeight: T.fontWeightSemibold,
                                color: T.black, letterSpacing: '-0.01em', lineHeight: 1.1 }}>
                    €{v}K
                  </div>
                  <div style={{ fontSize: 9, color: T.textMuted }}>
                    {c.deals}{' '}{c.deals === 1 ? 'deal' : 'deals'} · {pct}%
                  </div>
                </div>
              );
            })}
        ...
        {/* Totals row */}
        <div style={{
          fontSize: 10, color: T.black, fontWeight: T.fontWeightBold,
          padding: '6px 6px 0', borderTop: `1px solid ${T.black}`,
        }}>Total</div>
        {SEGMENTS.map(s => {
          const v = segTotals[s];
          const pct = Math.round((v / grandTotal) * 100);
          return (
            <div key={s} style={{
              padding: '6px 8px 0', borderTop: `1px solid ${T.black}`,
              display: 'flex', flexDirection: 'column', gap: 1, ...numStyle,
            }}>
              <div style={{ fontSize: 13, color: T.black, fontWeight: T.fontWeightBold,
                            letterSpacing: '-0.01em', lineHeight: 1.1 }}>€{v}K</div>
              <div style={{ fontSize: 9, color: T.textMuted }}>{pct}% of total</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ marginTop: 12, paddingTop: 8,
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          <span style={{ width: 10, height: 10, background: T.pleoPink100,
                         border: `1px solid ${T.pleoPink200}`, borderRadius: 2 }} />
          Low
        </span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          <span style={{ width: 10, height: 10, background: T.pleoPink400,
                         border: `1px solid ${T.pleoPink500}`, borderRadius: 2 }} />
          Critical
        </span>
        <span style={{ marginLeft: 'auto', ...numStyle }}>
          Total stale ARR: <strong style={{ color: T.black }}>€{grandTotal}K</strong>
        </span>
      </div>
    </Card>
  );
...
function PipelineQualityCharts({ data }) {
  ...
  return (
    ...
    <div style={{ background: T.card, borderRadius: 12, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 12 }}>
      {tab === 'slip' && (
        // 1/3 + 2/3 split — Slippage is a single-line trend, Stale concentration
        // is a wider Market × Segment grid that needs the room. Collapses to a
        // single column under ~720px so neither chart gets squashed on mobile.
        <div style={{ display: 'grid',
          ...
          <div style={{ minWidth: 0 }}>
            <SectionH meta="Market × Segment · €K stale ARR">Stale concentration</SectionH>
            <StaleBySegmentChart data={data} compact />
            <FindingBox>
              <strong style={{ color: T.black }}>Mid-Market holds 63% of all stale ARR</strong> —
              UK + NL Mid-Market alone is <strong style={{ color: T.black }}>36% (€495K across 12 deals)</strong>.
              The segment, not the rep, is the lever.
            </FindingBox>
          </div>
        </div>
      )}
